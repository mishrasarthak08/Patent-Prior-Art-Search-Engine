...
def test_fusion_recovers_missing_documents():
    ...
    doc_bm25 = RetrievedDocument(
        doc_id="doc_exact", title="Exact Match", snippet="...",
        retrieval_sources=["bm25"], raw_scores={"bm25": 10.0}, fused_score=0.0, matched_elements=[]
    )
    
    # Setup mock doc from Dense (Semantic Paraphrase) - BM25 misses this entirely
    doc_dense = RetrievedDocument(
        doc_id="doc_semantic", title="Semantic Paraphrase", snippet="...",
        retrieval_sources=["dense"], raw_scores={"dense": 0.9}, fused_score=0.0, matched_elements=[]
    )
...
def test_reranker_fallback_degrades_gracefully():
    ...
    doc = RetrievedDocument(
        doc_id="doc_1", title="Test", snippet="test snippet",
        retrieval_sources=["bm25"], raw_scores={}, fused_score=1.0, matched_elements=[]
    )
